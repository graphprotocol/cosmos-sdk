...
//this just CREATES the object to be stored in the store. look at the keeper to see the act
//of it getting stored
func (cm CounterMapper) NewEventObjectWithName(ctx sdk.Context, name string) GCI {
	event := cm.proto()
...
// Turn an name to key used to get it from the event store
...
func (cm CounterMapper) GetEvent(ctx sdk.Context, name string) GCI {
	...
	graphEventObject := cm.decodeCounter(bz)
	return graphEventObject
}
